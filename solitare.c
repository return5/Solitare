...
//print all the columns to screen.
void printAllColumns(void) {
	for(int i = 0; i < 7; i++) {
...
//if all columns are empty, presume player has won.
...
//check the suite of card 1 against card 2. 
int checkSuite(Card *const c1, Card *const c2) {
...
//move card from draw pile to somewhere else
void moveFromDrawPile(Column *const to,const int i,const int col_index) {
	const int bottom = to->bottom;
//if column is empy and selectd is king, or if bottom isnt empty, adn card on column is diffrent suite and correct value, then move card.
	if((bottom < 0 && Selected->value == 13)
		|| (bottom >= 0 && Selected->value == to->stack[bottom]->value + i && checkSuite(to->stack[bottom],Selected) == i))
   	{
...
void moveCard(Column *const from, Column *const to, const int a, const int col_index) {
	...
	if((bottom < 0 && Selected->value == 13)
		|| (bottom >= 0 && Selected->value == to->stack[bottom]->value + a && checkSuite(to->stack[bottom],Selected) == a ))
   	{
...
//delete windows before creating new ones when starting a new game
void deleteWin(void) {
	for(int i = 0; i < 7; i++) {
