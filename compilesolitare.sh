...
useGCC() {
    printf "%s\n" "using GCC"
    gcc -orogueclone -Wall -Wextra -O2 -Iheaders -finline-functions -Wswitch-enum -lncurses -std=gnu11 -osolitare
}

#compiles game using Clang
useClang() {
    printf "%s\n" "using Clang"
    clang -Iheaders -Wall -Wextra -O2 -lncurses -finline-functions -Wswitch-enum -std=gnu11 -osolitare   
}
